...
void main() async {
  ...
  group('All method test', () {
    test('all - licenses - success', () async {
      ...
      expect(record.elementAt(0).timestamp != null, true);
      expect(record.elementAt(1).timestamp != null, true);
    });
  ...
  group('Latest method test', () {
    test('latest - license - success', () async {
      ...
      LicenseRecord licenseRecord = await tikiTrail.license
          .create(titleRecord, uses, terms, expiry: DateTime(1));
      LicenseRecord? licenseRecordLatest =
          tikiTrail.license.latest(titleRecord);
    ...
    test('latest - license - for - a - title - with - no - licenses - success',
        () async {
      String ptr = const Uuid().v4();
      String origin = 'com.myco.myapp';
      List<TitleTag> tags = [TitleTag.contacts(), TitleTag.audio()];
    ...
    test(
        'latest - license - for - an - invalid - title - and - origin - success',
        () async {
      String invalidPtr = const Uuid().v4();
      String origin = 'com.myco.myapp';
      TitleRecord invalidTitleRecord =
          await tikiTrail.title.create(invalidPtr, origin: origin);
      LicenseRecord? licenseRecordLatest =
          tikiTrail.license.latest(invalidTitleRecord);
      expect(licenseRecordLatest, null);
    });
  });
}
